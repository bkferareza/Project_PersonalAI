    ...
    : IMachineBriefGenerator
{
    ...
    private const string BriefSystemMessage = """
        ...
        For any resource/learned-range comparison or authorized causal claim, copy the complete cited summary exactly as its own point, or omit it. Never infer that current resources are normal, within, above, or below learned ranges by comparing numbers yourself; even Established statistics alone do not supply a comparison.
        Keep resource comparisons and causal explanations out of overall. When copying a comparison summary into a point, do not rephrase it or append sample counts or conclusions. When causal language is not authorized, do not use "because", "due to", "requires", or "blocking", even if the evidence summary uses them; simply state the supplied status without a cause.
        Keep application failures and system shutdowns/restarts in separate statements with their own evidence IDs. Never transfer an aggregate system incident count to a named application.
        A pending restart is not a scheduled, queued, or automatic restart. Pending file rename operations do not prove that a restart is blocked or that updates are pending. State only the supplied pending status and reason without adding a causal explanation.
