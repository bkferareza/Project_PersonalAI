...
Overview is a quiet contextual home: Matasuri Brief leads, Today pairs observed PC energy and estimated cost with a plain-language learned comparison, one restrained machine status answers whether anything important is wrong, and Attention appears only for meaningful findings or insight. Raw CPU, memory, network, health, forecast, Learning, and runtime evidence remains on its dedicated page. History answers what happened. Learning answers what repeated evidence allowed Matasuri to conclude.
...
History opens with a shared local-day projection of observed PC energy and estimated electricity cost. It combines accepted additive History energy with the one pending valid contribution without double counting, uses only the matching effective-month published rate for historical periods, and survives session restarts. Overview presents that persistent Today status compactly without waking the model; it is not a household utility bill.
...
Matasuri owns a pinned official llama.cpp CUDA runtime and a checksum-verified Qwen3.5-4B model artifact. It never downloads either artifact during normal operation. The private server runs only as Matasuri's Job-Object-owned child, binds an authenticated random loopback port with no server UI, and starts only for an authorized local interpretation request. `Explain` interprets the selected deterministic Local Insight, while Matasuri Brief synthesizes what matters across the bounded whole-machine Situation. Brief generation is authorized only by a visible Overview with stale or missing material evidence or by `Refresh`; ambient telemetry, Running Bill updates, learning updates, and prose changes never wake the model or trigger New Insight.

The Brief uses a strict structured response with evidence identity on its overall assessment, each of at most three factual points, and its optional forward outlook. Deterministic validation rejects unknown evidence IDs, unsupported numbers or entities, unauthorized causal language, mutation advice, non-English output, and forecasts without supplied coverage. Resource/learned-range comparisons and authorized causal statements must reproduce a complete cited deterministic summary; matching numeric tokens alone does not establish a relationship. Application failures cannot be merged with system shutdown counts, and pending restart evidence does not establish a scheduled restart. One bounded repair receives the same machine evidence with an extractive selection/copy instruction instead of another free-form paraphrase; a second rejection produces a useful deterministic fallback. Verbatim deterministic summaries retain their source identity without treating ordinary title capitalization as a newly invented entity. A 60-minute material-Situation cache ignores tiny telemetry movement but invalidates for posture, significant finding, context, maturity, forecast, action-outcome, prompt, model, or runtime-version changes. C# remains authoritative for every fact and decision, and the shared local model uses a 10-minute idle residency before the private child unloads and releases model memory.
...
Electricity-rate enrichment attempts the narrow official source once per local calendar day, then uses verified cached evidence without retrying after either success or failure that day. Its only external requests are HTTPS requests to the selected coarse-location endpoint and an official utility source, both enforced by a component-local allowlist with no redirects. Coarse location is used only in memory to resolve a probable utility; IP addresses, coordinates, location history, account information, machine data, History, and Learning data are neither sent nor persisted. A published rate is retained only when its effective month, plausible total residential value, and source parse unambiguously. A failed refresh first uses a matching current-period rate and otherwise the latest verified official rate for current cost continuity; historical energy remains priced only by its matching effective-period evidence.
