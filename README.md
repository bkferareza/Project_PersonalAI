...
The Learning surface separates global memory health from the current hour/activity context's behavioral and power maturity. It shows samples, distinct observed days, total and Established context counts, and diagnostic recurring-pattern readiness: adjacent candidates, sample-qualified pairs, day-qualified pairs, fully eligible pairs, and the primary blocker. The generator still executes when PatternCount is zero; thresholds are not relaxed to manufacture maturity.
