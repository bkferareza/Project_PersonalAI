...
The ambient Windows presence includes a modern Mica-backed compact surface plus searchable, read-only inventories of classic desktop software, current-user MSIX/AppX packages, and startup applications. A platform-neutral deterministic findings layer evaluates verified CPU, memory, system-volume free space, and partial-data state; it drives the compact and expanded dashboard state. User-triggered local explanations begin with an exact deterministic opening, reject ungrounded model output into a verified fallback, and never send process telemetry, raw software inventories, startup names, commands, or paths to the model.
...
4. Trigger the breakpoint, such as by selecting **View dashboard**, then continue debugging normally.
...
Review findings coverage and select the first controlled maintenance action.
