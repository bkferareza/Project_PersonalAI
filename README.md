...
The ambient Windows presence now idles as a captionless 208×64 Desktop Acrylic capsule with only a living status core and a short deterministic phrase. Mouse or pen hover and keyboard focus reveal a 328×112 context view with the verified state, latest insight, and CPU and memory telemetry while preserving the bottom-right anchor; the whole surface opens the navigable expanded dashboard, and Back returns to the presence. State-specific motion is compositor-friendly, honors the Windows reduced-motion setting, and stops during shutdown. Searchable inventories remain read-only. Deterministic findings stay authoritative, while short local insights are generated proactively after stabilized state or finding changes, recovery, or the first dashboard open for a verified context. Context fingerprints, a two-minute automatic cooldown, one-request concurrency, and safe deterministic fallback prevent duplicate or ungrounded output; process telemetry, raw inventories, startup names, commands, and paths are never sent to the model.
...
1. Open `src/Machine.App/MainWindow.xaml.cs` and set a breakpoint in application code, such as the first line of `OnCompactPresenceClicked`.
